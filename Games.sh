 ...
 {Z} Retornar ao menu anterior \n"
  echo -n " => Escolha uma das opções: "
   ...
   case $REAL in
     ...
     h|H|"") [ -e ~/Downloads/open-ig-launcher.jar ] && echo -e "$WHEREISIT /Downloads! \n" && read -n1 -s -p "$CONTINUE" && Realtime || echo && cd ~/Downloads && wget https://github.com/akarnokd/open-ig/raw/master/open-ig-launcher.jar && mkdir Open_Imperium_Galactica && mv ~/Downloads/open-ig-launcher.jar ~/Downloads/Open_Imperium_Galactica && echo -e "$COMPLETED" && read -n1 -s -p "$CONTINUE" && Realtime ;;
